...
import { dirFileInterface, kindInterface, StateInterface } from "../../src/interface";
...
const FileType = ({ navigation, route } : any) => {
  ...
  const kindNeedRefresh = useSelector((state: StateInterface) => state.kindNeedRefresh);
    ...
    >
      <View style={{ ...styles.backgroundColorWhite, ...styles.padding10, ...styles.borderRadius10 }}>
        ...
        <View style={{...styles.fileList}}>
          {
            dirFileList.map((i: dirFileInterface) => <FileItem key={i.id} currentPath={'type'} fileItem={i} width={(windowWidth - 40) / 3} />)
          }
        </View>
        {
